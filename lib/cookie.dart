...
void set(String key, String value, {expires, path, domain, secure}) {
  if (expires is num) {
    expires = new DateTime.fromMillisecondsSinceEpoch(new DateTime.now().millisecondsSinceEpoch + expires*24*60*60*1000);
  }
  ...
  var cookie = ([
    ...
    expires != null ? '; expires=' + _format_date(expires) : '', // use expires attribute, max-age is not supported by IE
    path != null ? '; path=' + path : '',
...
bool remove(String key, {path, domain, secure}) {
  if (get(key) != null) {
    ...
    set(key, '', expires: -1, path: path, domain: domain, secure: secure);
    return true;
  }
  return false;
}
